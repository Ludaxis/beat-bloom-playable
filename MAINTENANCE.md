...
Before release, test on physical lower-end Android and iOS devices and in each network's validator/SDK. Desktop CPU throttling is useful regression evidence, but does not reproduce mobile GPU performance, thermal limits or audio output latency. The local export server is not a Vercel serverless deployment.
